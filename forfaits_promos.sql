...
insert into public.forfaits (slug, nom, prix, duree_jours, actif) values
  ...
  ('premium', 'Premium annuel', 3000, 365, true),
  ('premium_mensuel', 'Premium mensuel', 1000, 30, true)
on conflict (slug) do nothing;

-- Si ce script a déjà été exécuté une première fois (ancien tarif 10 000 FCFA),
-- la ligne 'premium' existe déjà et l'insert ci-dessus ne la modifie pas
-- (ON CONFLICT DO NOTHING). Cette mise à jour corrige son prix vers le
-- nouveau tarif annuel (3 000 FCFA), sans toucher au forfait mensuel.
update public.forfaits set prix = 3000 where slug = 'premium';
...
-- NOTE — Essai gratuit du 1er mois : dans cette version, l'essai gratuit
-- (30 jours à la création du compte) est calculé côté application à partir
-- de la date de création du compte, sans dépendre de premium_expire. Cette
-- colonne reste disponible pour un futur suivi côté serveur de l'abonnement
-- payant si besoin.
